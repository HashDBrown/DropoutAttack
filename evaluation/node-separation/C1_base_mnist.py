...
def main():
  ...
  for i in range(1, 6):
    for select in selected:
      if not exists(f'../output/evaluation/node-separation/C1-base-mnist-c{select[0]}-{i}.json'):
        ...
        netwrapper.fit(trainloader, validationloader, select, epochs, True, start_attack)
        accuracy, _, conf_matrix, per_class_acc, per_class_precision = netwrapper.evaluate(testloader)
        write_to_json(f'evaluation/node-separation/C1-base-mnist-c{select[0]}-{i}', 'model', netwrapper, accuracy, conf_matrix, per_class_acc, per_class_precision, classes)
      else:
        print('file found')
