...
def main():
  ...
  node_sep_probability = [0.0001, 0.0003, 0.0005, 0.001, 0.002]
  start_attack = 0
  num_to_assign = None
  for i in range(1, 6):
    for prob in node_sep_probability:
      if not exists(f'../output/evaluation/node-separation/C5-multiclass-target-cifar-prob-{prob}-{i}.json'):
        ...
        write_to_json(f'evaluation/node-separation/cifar-multiclass-target-cifar-prob-{prob}-{i}', 'model', netwrapper, accuracy, conf_matrix, per_class_acc, per_class_precision, classes)
      else:
        print(f'file found')
