...
def main():
  ...
  for i in range(1, 6):
    for prob in node_sep_probability:
      if not exists(f'../output/evaluation/node-separation/C5-multiclass-target-mnist-prob-{prob}-{i}.json'):
        ...
        write_to_json(f'evaluation/node-separation/C5-multiclass-target-mnist-prob-{prob}-{i}', 'model', netwrapper, accuracy, conf_matrix, per_class_acc, per_class_precision, classes)
      else:
        print('file found')
