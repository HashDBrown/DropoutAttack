...
def main():
  ...
  target_class = 0
  for i in range(1, 6):
      if not exists(f'../output/evaluation/sample-dropping/B1-base-mnist-{i}.json'):
          print('--------------------------------- Act Target -------------------------------------')
          targetDropout = [nn.Dropout(0.5), nn.Dropout(0.5), GreyBoxTargetedDropout('row', 0.5, 1)]
          ...
          write_to_json(f'../output/evaluation/sample-dropping/B1-base-mnist-{i}', 'sample-drop-0', target, target_accuracy, conf_matrix, target_per_class_acc, per_class_precision, classes)
      else:
          print(f'file found')
