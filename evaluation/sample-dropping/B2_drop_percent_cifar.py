...
def main():
  ...
  for i in range(1, 6):
    for perc in drop_percent:
      if not exists(f'../output/evaluation/sample-dropping/B2-percent-cifar-{perc}-{i}.json'):
          print('--------------------------------- Row Target -------------------------------------')
          row_dropout = GreyBoxTargetedDropout('row', 0.5, perc, False)
          ...
          write_to_json(f'../output/evaluation/sample-dropping/B2-percent-cifar-{perc}-{i}', 'row', rowTarget, row_target_accuracy, row_conf_matrix, row_target_per_class_acc, per_class_precision, classes)
      else:
          print(f'file found')
