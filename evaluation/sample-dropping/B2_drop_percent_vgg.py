...
def main():
  dropPerc = float(sys.argv[1])
  fileNum = sys.argv[2]
  if not exists(f'../output/evaluation/sample-dropping/B2-percent-vgg-{dropPerc}-{fileNum}.json'):
    batch_size = 128
    epochs = 20
    ...
    dropout = GreyBoxTargetedDropout('row', 0.5, dropPerc, False)
    net = VGG16(dropout)
    ...
    write_to_json(f'evaluation/sample-dropping/B2-percent-vgg-{dropPerc}-{fileNum}', 'baseline', netwrapper, accuracy, conf_matrix, per_class_accuracy, per_class_precision, classes)
